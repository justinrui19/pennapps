...
# Defaults
STREAM_URL_DEFAULT = get_default_stream_url()
CAMERA_BASE_DEFAULT = get_default_camera_base()


# Camera control helpers use the port 80 base
...
if __name__ == '__main__':
    ...
    parser.add_argument('--post-interval', type=float, default=0.5, help='Seconds between detection posts')
    parser.add_argument('--stream', type=str, default=STREAM_URL_DEFAULT, help='ESP32 MJPEG stream URL, e.g. http://ip:81/stream')
    parser.add_argument('--camera-base', type=str, default=CAMERA_BASE_DEFAULT, help='ESP32 base URL for camera controls (port 80)')
    ...
    # Configure camera
    set_resolution(args.camera_base, index=6)  # Use VGA (640x480)
    ...
    while True:
        if cap.isOpened():
            ...
            if ret and frame is not None and frame.size > 0:
                ...
                # Publish detections to server periodically
                if args.server and (current_time - last_post_time) >= args.post_interval:
                    try:
                        requests.post(
                            f"{args.server.rstrip('/')}/api/detections",
                            params={"replace": "true"},
                            json=last_detections,
                            timeout=0.5
                        )
            ...
            if key == ord('r'):
                ...
                set_resolution(args.camera_base, index=idx, verbose=True)

            elif key == ord('q'):
                val = int(input("Set quality (10 - 63): "))
                set_quality(args.camera_base, value=val)

            elif key == ord('a'):
                AWB = set_awb(args.camera_base, 1)
